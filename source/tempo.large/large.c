...
int C74_EXPORT main(void)
{
  ...
  post("External code by Jaime Chao based on a patch by Laurent Bonasse-Gahot");
  post("Corrections and updates by Benjamin Lévy");
  post("Compiled on %s at %s", __DATE__, __TIME__);
  ...
  CLASS_ATTR_DOUBLE(c, "shift", 0, t_large, shift);
  CLASS_ATTR_FILTER_CLIP(c, "shift", -0.5, 0.5);
  ...
  if ((x = (t_large *)object_alloc(large_class)))
  {
    ...
    if(argc == 1) {
      if (atom_gettype(argv) == A_FLOAT)
        ...
        object_error((t_object*)x, "Argument must be a float, using default period: 500ms");
        x->initPeriod = 500.;
      }
    }
    else
    {
      //object_post((t_object*)x, "Using default period: 500ms");
      x->initPeriod = 500.;
    }
...
void large_bang(t_large *x)
{
  ...
  double shifted_phase = x->phase + x->shift;
  shifted_phase = WRAP(shifted_phase) ;
  outlet_float(x->outlet2, shifted_phase);
  
  //output a pulse in left outlet
  if ( shifted_phase == 0. || (mem + x->shift < 0. && shifted_phase > 0.))
...
void large_reset(t_large *x)
{
  ...
  outlet_float(x->outlet3, x->period);
  outlet_float(x->outlet2, x->phase);
//  post("gamma %f  etaP %f  etaPhi %f shift %f", x->gamma, x->eta_p, x->eta_phi, x->shift);
}
